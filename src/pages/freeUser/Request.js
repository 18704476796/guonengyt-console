...
export default {
  ...
  onlocked (_this, locked, userName) { // 修改用户状态函数
    ...
    Passenger.updateSysUserStatus(_this, formData, 2)
  },
  lookFor (_this, id) { // 查看免费体验用户详情
    let formData = {
      userId: id
    }
    Passenger.lookFor(_this, formData, 2)
  },
  onSubmit (_this) { // 创建管理员函数
    _this.$refs.ruleForm.validate((valid) => {
      if (valid) { // 表单验证通过
        ...
        Passenger.saveOrUpdateSysUser(_this, formData, 2)
      } else {
  ...
  deleteUser (_this, userName) { // 删除乘客用户函数
    ...
    Passenger.deleteSysUser(_this, formData, 2)
  },
