...
export default {
  ...
  onlocked (_this, locked, userName) { // 修改用户状态函数
    ...
    Passenger.updateSysUserStatus(_this, formData, 3)
  },
  lookFor (_this, id) { // 查看加入我们用户详情
    let formData = {
      userId: id
    }
    Passenger.lookForJoinUser(_this, formData, 3)
  },
  onSubmit (_this) { // 创建管理员函数
    _this.$refs.ruleForm.validate((valid) => {
      if (valid) { // 表单验证通过
        ...
        Passenger.saveOrUpdateSysUser(_this, formData, 3)
      } else {
  ...
  deleteUser (_this, userName) { // 删除乘客用户函数
    ...
    Passenger.deleteSysUser(_this, formData, 3)
  },
