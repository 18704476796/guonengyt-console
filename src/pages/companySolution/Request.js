...
export default {
  getInfo (_this, pageSize, currentPage, locked, searchType, searchValue) { // 解决问题初始化页面
    ...
    Solution.findSolutionsList(_this, formData, 1)
  },
  ...
  lookFor (_this, id) { // 查看免费体验用户详情
    ...
    Solution.lookFor(_this, formData, 1)
  },
  onSubmit (_this) { // 新增解决方案
    _this.$refs.ruleForm.validate((valid) => {
      if (valid) { // 表单验证通过
        ...
        Solution.saveOrUpdateSolution(_this, formData, 1)
      } else {
  ...
  deleteUser (_this, id) { // 删除解决方案
    _this.$confirm('此操作将永久删除该条信息, 是否继续?', '提示', {
      ...
      Solution.deleteSysUser(_this, formData, 1)
    }).catch(() => {
      _this.$message({
        type: 'info',
        message: '已取消删除'
