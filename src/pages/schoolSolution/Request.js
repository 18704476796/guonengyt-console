...
export default {
  getInfo (_this, pageSize, currentPage, locked, searchType, searchValue) { // 解决问题初始化页面
    ...
    Solution.findSolutionsList(_this, formData, 2)
  },
  lookFor (_this, id) { // 查看免费体验用户详情
    let formData = {
      newsDataId: id
    }
    Solution.lookFor(_this, formData, 2)
  },
  onlocked (_this, locked, id) { // 修改解决方案状态函数
    ...
    Solution.updateSysUserStatus(_this, formData, 2)
  },
  onSubmit (_this) { // 新增解决方案
    _this.$refs.ruleForm.validate((valid) => {
      if (valid) { // 表单验证通过
        ...
        Solution.saveOrUpdateSolution(_this, formData, 2)
      } else {
  ...
  deleteUser (_this, id) { // 删除解决方案
    _this.$confirm('此操作将永久删除该条信息, 是否继续?', '提示', {
      ...
      Solution.deleteSysUser(_this, formData, 2)
    }).catch(() => {
      _this.$message({
        type: 'info',
        message: '已取消删除'
